...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="SBML -> parameter-extraction-manifest")
    ...
    if sbml_path is None or not sbml_path.exists():
        print("error: provide --sbml-path or working --biomodels-id + --download-to", file=sys.stderr)
        return 2
    ...
    header = ManifestHeader(
        doi=args.paper_doi,
        biomodels_id=args.biomodels_id,
    ...
    print("  4. Run: python tools/curate_params.py --manifest <path>  (when biology-curator is built)")
    return 0
