"""Diagnose the (seed=20, tick=16), j=417 MIN GLP_NOFEAS failure under the
basis-reset-per-solve approach. Does a fresh advanced basis sometimes lead to
a FALSE infeasibility detection (numerical issue with the crash heuristic /
phase-1) on this ill-scaled network, even though the biomass-optimal face is
provably non-empty (it contains the optimal solution found by the primary
solve)?
"""
...
def make_parm(tm_lim_ms, pricing):
    parm = glp.glp_smcp()
...
def main():
    ...
    # Cold, standalone solve of j=417 MIN with a fresh crash basis -- isolate
    # whether the NOFEAS is inherent to the LP-plus-crash-basis combination,
    # or only appears in the sequence context.
    for pricing_name, pricing in (("PSE", glp.GLP_PT_PSE), ("STD", glp.GLP_PT_STD)):
        lp = build_lp(S, rhs, c, lb, ub, m_rows, n_rxn)
        glp.glp_adv_basis(lp, 0)
        parm = make_parm(10_000, pricing)
        exit_code = glp.glp_simplex(lp, parm)
        print(f"[{pricing_name}] primary solve: exit={exit_code} status={glp.glp_get_status(lp)} "
              f"it_cnt={glp.glp_get_it_cnt(lp)}")
        ...
        it_cnt = glp.glp_get_it_cnt(lp)
        val_ = glp.glp_get_col_prim(lp, j + 1) if status == glp.GLP_OPT else None
        print(f"[{pricing_name}] cold MIN j={j}: exit={exit_code} status={status} "
              f"it_cnt={it_cnt} value={val_}")

        # Try again with presolve ON (GLPK's presolve is designed to be more
        # numerically robust against exactly this kind of false-infeasibility
        # detection on badly-scaled/degenerate problems).
        parm2 = make_parm(10_000, pricing)
        parm2.presolve = glp.GLP_ON
        exit_code2 = glp.glp_simplex(lp, parm2)
        status2 = glp.glp_get_status(lp)
        val2 = glp.glp_get_col_prim(lp, j + 1) if status2 == glp.GLP_OPT else None
        print(f"[{pricing_name}] presolve=ON retry MIN j={j}: exit={exit_code2} "
              f"status={status2} value={val2}")

        # Try relaxing the FX row to a numerically-tiny DB (double-bounded)
        # window instead of an exact equality, to test whether the NOFEAS is
        # purely a floating-point equality-tolerance artifact (not a genuine
        # infeasibility).
        eps_numeric = max(1e-9, abs(biomass_value_star) * 1e-9)
        glp.glp_set_row_bnds(
            lp, biomass_row, glp.GLP_DB,
            biomass_value_star - eps_numeric, biomass_value_star + eps_numeric,
        )
        glp.glp_adv_basis(lp, 0)
        exit_code3 = glp.glp_simplex(lp, parm)
        status3 = glp.glp_get_status(lp)
        val3 = glp.glp_get_col_prim(lp, j + 1) if status3 == glp.GLP_OPT else None
        print(f"[{pricing_name}] tiny-DB-window retry MIN j={j}: exit={exit_code3} "
              f"status={status3} value={val3} (eps_numeric={eps_numeric!r})")
