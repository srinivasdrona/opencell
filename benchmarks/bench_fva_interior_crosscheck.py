"""Independent cross-check of the 3 columns where GLP_PT_STD hung
(timed out at the 60s/column safety cap) on sample seed=0,tick=5:
j=3 (min), j=66 (max), j=233 (max).

Uses GLPK's interior-point method (glp_interior), a completely different
algorithm family (barrier method, no basis/pivoting, cannot exhibit simplex
cycling) to independently confirm the PSE-pricing simplex answer is the true
LP optimum, not an artifact of the pricing-rule change.

Run via: bin\\oc-py benchmarks\\bench_fva_interior_crosscheck.py
"""
...
def add_face(glp, lp, biomass_value_star, c):
...
def main() -> None:
    ...
    targets = [(3, "min"), (66, "max"), (233, "max")]
    # PSE simplex answers from bench_fva_pricing_fix.py, for reference in output.
    pse_answers = {(3, "min"): 2758.04794989159, (66, "max"): 22.662232275499353, (233, "max"): -288.1189798573032}

    for j, direction in targets:
        lp = build_lp(glp, S, rhs, c, lb, ub)
        parm = glp.glp_smcp()
        ...
        add_face(glp, lp, biomass_value_star, c)
        glp.glp_set_obj_coef(lp, j + 1, 1.0)
        glp.glp_set_obj_dir(lp, glp.GLP_MAX if direction == "max" else glp.GLP_MIN)

        # Independent solve via interior-point (barrier) method -- structurally
        # cannot cycle since it has no basis/pivoting; different code path entirely.
        iparm = glp.glp_iptcp()
        glp.glp_init_iptcp(iparm)
        iparm.msg_lev = glp.GLP_MSG_OFF
        ip_ret = int(glp.glp_interior(lp, iparm))
        ip_status = int(glp.glp_ipt_status(lp))
        ip_val = float(glp.glp_ipt_col_prim(lp, j + 1)) if ip_status == glp.GLP_OPT else float("nan")

        pse_val = pse_answers[(j, direction)]
        diff = abs(ip_val - pse_val)
        rel_ok = diff <= 1e-3 * max(1.0, abs(pse_val))  # interior point stops at a duality-gap tolerance, not exact
        print(
            f"j={j} dir={direction}: interior_point_ret={ip_ret} status={ip_status} val={ip_val:.6f} "
            f"| PSE_simplex_val={pse_val:.6f} | diff={diff:.2e} {'OK (independent methods agree)' if rel_ok else 'MISMATCH'}"
        )
