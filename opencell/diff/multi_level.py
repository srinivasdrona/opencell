...
class DiffReport:
    ...
    def passed(self) -> bool:
        ...
        if self.level2_a_invariants and not self.level2_a_invariants.passed:
            return False
        return not (self.level2_b_invariants and not self.level2_b_invariants.passed)
...
def _navigate(traj: TrajectoryDict, path: PortPath) -> object | None:
    node = traj
    for k in path:
...
def build_default_invariant_suite(
    ...
    if by_kind["signal"]:
        ...
        suite.add(
            lambda v=vals, b=bounds: check_bounded(
                times=times,
                values=v,
                bounds=b,
                name="signal_bounds",
    ...
    if by_kind["count"]:
        vals = by_kind["count"]
        suite.add(
            lambda v=vals: check_non_negativity(
                times=times,
                values=v,
                name="count_non_negativity",
            )
        )
        suite.add(
            lambda v=vals: check_count_integrality(
                times=times,
                values=v,
            )
        )
...
def diff_phenotypes(
    traj_a: TrajectoryDict,
    traj_b: TrajectoryDict,
    spec: DiffSpec,
    ...
    keys = [k for k in keys if not spec.scalar_phenotypes or k in spec.scalar_phenotypes]
    for k in keys:
        va = pa.get(k)
        vb = pb.get(k)
...
def run_diff(
    ...
    *,
    ...
    suite_a = invariant_suite_a or build_default_invariant_suite(
        f"{spec.engine_a_name}_invariants",
        traj_a,
        spec,
    )
    suite_b = invariant_suite_b or build_default_invariant_suite(
        f"{spec.engine_b_name}_invariants",
        traj_b,
        spec,
    )
    ...
    report.level4_findings = diff_phenotypes(
        ...
        abs_tol=phenotype_abs_tol,
        rel_tol=phenotype_rel_tol,
    )
    return report
