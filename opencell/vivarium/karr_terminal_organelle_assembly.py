...
class KarrTerminalOrganelleAssemblyProcess(Process):
    ...
    def _load_substrate_projection_schema(self, path: str | Path) -> None:
        ...
        # Day-37 fix: fall back to v2.1 schema format if legacy [substrates]
        # section is missing. The autogenerated schema (schema_version 2.1+)
        # uses [state_groups] and [observables] instead of [substrates].
        # See data/schemas/per_process/terminal_organelle_assembly.toml.
        if not schema_wids or not compartment_wids:
            state_groups = parsed.get("state_groups", {})
            observables = parsed.get("observables", {})
            substrate_group = state_groups.get("substrates", [])
            substrate_obs = observables.get("substrates", {})
            substrate_shape = substrate_obs.get("shape", [])
            if substrate_group and substrate_shape and len(substrate_shape) == 2:
                schema_wids = tuple(str(wid) for wid in substrate_group)
                # Compartment names not in v2.1 schema; use generic names that
                # match the OC chassis convention (e.g., "compartment_0"...).
                # Semantic compartment names (incorporated/unincorporated for
                # TerminalOrganelleAssembly) live in fixture metadata if needed.
                n_compartments = int(substrate_shape[0])
                compartment_wids = tuple(
                    f"compartment_{idx}" for idx in range(n_compartments)
                )

        if not schema_wids or not compartment_wids:
            raise ValueError(
                "TerminalOrganelleAssembly schema requires substrates.wids and "
                "substrates.compartment_wids (or v2.1 state_groups.substrates + "
                "observables.substrates.shape)"
            )
