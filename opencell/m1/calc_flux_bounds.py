...
DEFAULT_FIXTURE_JSON = (
    ...
    / "data" / "karr_fixtures" / "karr_native_m1_dynamics.json"
)
    ...
    *,
    ...
    if apply_protein_bounds:
        raise NotImplementedError(
            "rule 6 (protein bounds) deferred to Phase C")

    n_rxn = catalysis.shape[0]
    if catalysis.shape != (n_rxn, enzymes.size):
        raise ValueError(
            f"catalysis {catalysis.shape} vs enzymes {enzymes.size}")
    ...
    if apply_external_metabolite:
        ...
        lower[ext_rxn] = np.fmax(
            lower[ext_rxn], fba_reaction_bounds[ext_rxn, 0] * cell_dry_mass)
        upper[ext_rxn] = np.fmin(
            upper[ext_rxn], fba_reaction_bounds[ext_rxn, 1] * cell_dry_mass)
    ...
    if apply_internal_metabolite:
        ...
        if int_rxn.size != int_sub.size:
            raise ValueError(
                f"internal-lim rxn count {int_rxn.size} != sub count {int_sub.size}")
        ...
        lower[int_rxn] = np.fmax(
            lower[int_rxn], -cytosol_counts / step_size_sec)
