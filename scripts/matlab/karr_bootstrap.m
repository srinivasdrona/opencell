...
mnrnd_provider = require_genuine_statistics_mnrnd(repo_root);
...
function provider = require_genuine_statistics_mnrnd(repo_root)
% require_genuine_statistics_mnrnd  Fail closed unless the real MathWorks
% Statistics and Machine Learning Toolbox mnrnd is available and resolved
% ahead of the repo shim even after repo paths were added.
shim_path = fullfile(repo_root, 'scripts', 'matlab', 'mnrnd.m');
...
if license_test ~= 1 || license_checkout ~= 1
    error('karr_bootstrap:statistics_toolbox_unavailable', ...
        ['full-simulation extraction requires the genuine Statistics and Machine Learning Toolbox ' ...
         'mnrnd.m; license(''test'',''Statistics_Toolbox'')=%d, license(''checkout'',''Statistics_Toolbox'')=%d. ' ...
         'The repo shim %s is prohibited as Karr evidence.'], ...
        license_test, license_checkout, shim_path);
end
...
if isempty(stats_info)
    error('karr_bootstrap:statistics_toolbox_missing', ...
        ['full-simulation extraction requires ver(''stats'') to resolve the Statistics and ' ...
         'Machine Learning Toolbox; none was found. The repo shim %s is prohibited as Karr evidence.'], ...
        shim_path);
end
stats_info = stats_info(1);

expected_provider_path = fullfile(matlabroot, 'toolbox', 'stats', 'stats', 'mnrnd.m');
if exist(expected_provider_path, 'file') ~= 2
    error('karr_bootstrap:statistics_mnrnd_missing', ...
        ['full-simulation extraction requires the genuine MathWorks provider at %s, ' ...
         'but that file is missing. The repo shim %s is prohibited as Karr evidence.'], ...
        expected_provider_path, shim_path);
end

addpath(fileparts(expected_provider_path), '-begin');
rehash;
resolved_provider_path = which('mnrnd');
if isempty(resolved_provider_path)
    error('karr_bootstrap:statistics_mnrnd_unresolved', ...
        ['full-simulation extraction could not resolve mnrnd after promoting %s. ' ...
         'The repo shim %s is prohibited as Karr evidence.'], ...
        expected_provider_path, shim_path);
end
if ~same_path(resolved_provider_path, expected_provider_path)
    error('karr_bootstrap:statistics_mnrnd_shadowed', ...
        ['full-simulation extraction resolved mnrnd to %s instead of the required Statistics ' ...
         'Toolbox provider %s. The repo shim %s must never be used.'], ...
        resolved_provider_path, expected_provider_path, shim_path);
end
...
provider = struct( ...
    ...
    'provider_path_relative_to_matlabroot', relative_to_matlabroot(resolved_provider_path), ...
    'sha256_lf_normalized', sha256_lf_normalized(resolved_provider_path) ...
);
end
