...
def main() -> int:
    ...
    checks.append(
        _check(
            ...
            detail={"t_end_emit": float(t[-1]), "t_end_request": T_END_S, "n_emit": int(len(t))},
        )
    )
    ...
    checks.append(
        _check(
            ...
            detail={
                "emitted": [rxn_emit, rna_emit, prot_emit],
    ...
    checks.append(
        _check(
            "C6_growth_stable",
            ok=(g_range < 1e-9),
            detail={"growth_per_h_initial": float(g[0]), "growth_per_h_range_after_t1": g_range},
        )
    )
    ...
    checks.append(
        _check(
            ...
            detail={
                "rna_max_range": rna_max_range,
                "rna_worst": rna_worst_id,
                "prot_max_range": prot_max_range,
                "prot_worst": prot_worst_id,
            },
        )
    )
    ...
    aa_total_series = sum(np.asarray(ts["substrates"][aa], dtype=float) for aa in tl.AA_WCM_IDS)
    float(aa_total_series[-1] - aa_total_series[0])
    checks.append(
        _check(
            "C4_aa_per_aa_writeback_vs_helper",
            ok=aa_ok,
            detail={
                "per_aa_observed": per_aa_observed,
    ...
    checks.append(
        _check(
            ...
            detail={
                "total_nt_per_s_from_predictor": total_nt_v2,
    ...
    checks.append(
        _check(
            ...
            detail={
                "total_aa_per_s_from_predictor": total_aa_v2,
    ...
    pred_gene_cycle = txv2.predict_gene_synthesis_per_s(
        m2v2,
        n_active=2.0 * m2v2.n_active_rnap,
    )
    summary_snap = txv2.compare_to_karr(
        pred_gene_snapshot,
        m2v2.karr_fitted_synth_per_s,
    )
    ...
    checks.append(
        _check(
            ...
            ok=(summary_cycle["median_abs_log2_ratio"] < summary_snap["median_abs_log2_ratio"]),
            detail={"snapshot": summary_snap, "cycle_2x": summary_cycle},
        )
    )

    # -- C10 M3 v1 vs v2 ratio (documented ~23x gap) ---------------------
    karr_total_v1 = float(np.sum(m3v2.karr_v1_synth_per_s * m3v2.length_aa))
    ...
    rna_max_dev = max(
        abs(rna0[g] - float(m2.expression[i, CONDITION])) for i, g in enumerate(m2.gene_wcm_ids)
    )
    prot_max_dev = max(
        abs(prot0[p] - float(m3.counts_mature[i])) for i, p in enumerate(m3.protein_wcm_ids)
    )
    checks.append(
        _check(
            ...
            detail={
                "rna_max_abs_dev_from_expression": rna_max_dev,
    ...
    for ntp, color in [
        ...
        ax.plot(
            ...
            ls=":",
            alpha=0.6,
            label=f"{ntp} predicted ({slope:.4g}/s)",
        )
    ...
    ax = axes[1]
    ax.plot(t, aa_total_series, color="tab:purple", lw=1.6, label="Sum 20 AAs (engine)")
    aa_slope = -aa_helper["_total_aa_per_s"]
    ax.plot(
        t,
        aa_total_series[0] + aa_slope * t,
        color="tab:purple",
        lw=0.8,
        ls=":",
        alpha=0.7,
