...
def check_g2_replay_test(process_key: str) -> tuple[bool, str, str]:
    ...
    for c in candidates:
        if c.exists():
            return True, str(c.relative_to(ROOT)), "exact-name match"
    # Heuristic: grep for "replay" referencing this process key
    hits: list[str] = []
    ...
    if hits:
        return True, hits[0], f"heuristic: {len(hits)} file(s) reference process+replay"
