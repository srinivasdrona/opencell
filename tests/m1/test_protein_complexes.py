...
def test_loads_201_complexes(model):
    assert len(model.all_wids()) == 201
    assert model.counts["n_complexes"] == 201


def test_compartment_vocab_has_six_wids(model):
    assert set(model.compartment_wids) == {"c", "d", "e", "m", "tc", "tm"}


def test_every_complex_has_at_least_one_participant(model):
    """A protein complex must assemble from something."""
    bare = []
    for wid, c in model.complexes.items():
        n_parts = (
            len(c.monomers) + len(c.subcomplexes) + len(c.metabolites)
            + len(c.prosthetic) + len(c.chaperones) + len(c.rnas)
        )
        if n_parts == 0:
            bare.append(wid)
...
def test_dna_gyrase_a2b2(model):
    c = model["DNA_GYRASE"]
...
def test_dna_gyrase_flatten_to_monomers(model):
...
def test_rna_polymerase_4_distinct_subunits(model):
...
def test_ribosome_30s_has_20_proteins_and_1_rrna(model):
...
def test_ribosome_50s_has_32_proteins_and_2_rrnas(model):
    c = model["RIBOSOME_50S"]
    assert len(c.monomers) == 32
    assert len(c.rnas) == 2


def test_ribosome_70s_is_30s_plus_50s(model):
...
def test_ribosome_70s_flatten_to_monomers_aggregates(model):
...
def test_ribosome_70s_flatten_full_includes_rrnas(model):
...
def test_monomers_required_multiplies_demand(model):
...
def test_some_complexes_have_metabolites(model):
...
def test_some_complexes_have_subcomplexes(model):
...
def test_complex_unique_wids(model):
    wids = model.all_wids()
