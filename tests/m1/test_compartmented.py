...
def test_S_shape_585_645_3(model) -> None:
...
def test_aggregate_matches_sum_over_compartments(model) -> None:
    np.testing.assert_array_equal(model.S_aggregate, model.S.sum(axis=2))


def test_compartment_vocab_is_c_e_m(model) -> None:
...
def test_unknown_compartment_raises(model) -> None:
    with pytest.raises(KeyError):
        model.compartment_index("d")  # DNA compartment not in metabolism


def test_substrate_and_reaction_counts(model) -> None:
...
def test_S_is_signed_integer_dtype(model) -> None:
    assert np.issubdtype(model.S.dtype, np.integer)


def test_some_stoichiometry_is_nonzero(model) -> None:
    assert model.stats["nnz_total"] > 1000


def test_cytosol_has_most_nonzeros(model) -> None:
...
def test_glucose_uptake_sign_convention(model) -> None:
...
def test_dna_gyrase_atp_hydrolysis(model) -> None:
...
def test_mmol_per_gdwh_to_molecules_per_s(model) -> None:
...
def test_unit_conversion_is_linear(model) -> None:
...
def test_baseline_is_nonempty_dict(baseline) -> None:
    assert isinstance(baseline, dict)
    assert len(baseline) > 20, f"baseline has only {len(baseline)} entries"


def test_baseline_keys_are_substrate_compartment_pairs(baseline, model) -> None:
...
def test_baseline_includes_extracellular_glucose_uptake(baseline) -> None:
...
def test_baseline_NTPs_NOT_supplied_through_FBA(baseline) -> None:
    ...
    for ntp in ("ATP", "CTP", "GTP", "UTP"):
        ...
        if rate is not None:
            ...
            assert rate < 1e6, f"unexpected large positive {ntp} supply through FBA: {rate}"


def test_baseline_total_carbon_is_negative(baseline) -> None:
