...
def _has_perturbation_oracle() -> bool:
    """Check whether the MATLAB perturbation panel oracle exists.

    `data/m1_sources/karr_flat/metabolism_dynamics.mat` is generated by a
    MATLAB-side fixture-extraction step that hasn't been wired into the
    Python bootstrap. Tests that depend on it should skip when missing
    rather than fail — they are oracle-comparison tests, not regressions.
    """
    return Path("data/m1_sources/karr_flat/metabolism_dynamics.mat").exists()


_SKIP_PERTURBATION = pytest.mark.skipif(
    not _has_perturbation_oracle(),
    reason="MATLAB perturbation oracle missing: data/m1_sources/karr_flat/metabolism_dynamics.mat. "
           "Regenerate via the MATLAB side or skip these oracle tests.",
)
