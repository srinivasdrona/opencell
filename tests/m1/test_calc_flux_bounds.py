...
def _bound_diff(a: np.ndarray, b: np.ndarray) -> dict:
    ...
    n_inf_signed_mismatch = int((np.isinf(a) & np.isinf(b) & (np.sign(a) != np.sign(b))).sum())
    return {
...
def test_loader_dimensions(dyn) -> None:
...
def test_compartment_extracellular_is_index_1(dyn) -> None:
    """Karr's MATLAB has compartmentIndexs_extracellular = 2 (1-based);
    0-based should be 1."""
    assert dyn.compartment_extracellular_0based == 1


def test_substrate_indexs_fba_unique_and_in_range(dyn) -> None:
    pairs = list(
        zip(dyn.substrate_idx_fba_sub0.tolist(), dyn.substrate_idx_fba_cmp0.tolist(), strict=False)
    )
...
def test_compute_bounds_matches_matlab_oracle_no_protein(m1, dyn) -> None:
...
def test_rule3_off_widens_bounds(m1, dyn) -> None:
...
def test_rule4_zero_external_substrate_zeros_uptake(m1, dyn) -> None:
...
def test_rule5_zero_internal_lim_substrate_zeros_lower(m1, dyn) -> None:
...
def test_rule1_zero_enzyme_zeros_catalysed_reactions(m1, dyn) -> None:
...
def test_rule6_protein_bounds_raises() -> None:
    """Rule 6 is intentionally not implemented in Phase A."""
    with pytest.raises(NotImplementedError):
        cfb.compute_bounds(
            substrates=np.zeros((585, 3)),
            enzymes=np.zeros(104),
            cell_dry_mass=1.0,
            step_size_sec=1.0,
            catalysis=np.zeros((504, 104)),
            enz_bounds=np.zeros((504, 2)),
...
def _load_perturbation_oracle(panel: str) -> np.ndarray:
    """Load perturbation panel bounds from MATLAB v7.3 output.

    MATLAB can emit either `#refs#/panel/bounds` or `panel/bounds`
    depending on how structs are laid out in the HDF5 file.
    """
    import h5py

    mat_path = Path("data/m1_sources/karr_flat/metabolism_dynamics.mat")
    with h5py.File(mat_path, "r") as f:
        refs_path = f"#refs#/{panel}/bounds"
        root_path = f"{panel}/bounds"
        if refs_path in f:
            arr = np.array(f[refs_path])
        elif root_path in f:
            arr = np.array(f[root_path])
        else:
            raise KeyError(
                f"Missing perturbation oracle for panel '{panel}' in {mat_path}. "
                f"Tried '{refs_path}' and '{root_path}'."
            )
    return arr.T


def test_perturbation_panel_p1_matches_oracle(m1, dyn) -> None:
    ...
    mat_b = _load_perturbation_oracle("b")
    diff = _bound_diff(py, mat_b)
    assert diff["n_inf_mismatch"] == 0, diff
    assert diff["finite_max_abs_diff"] < 1e-9, diff


def test_perturbation_panel_p2_matches_oracle(m1, dyn) -> None:
    """MATLAB perturbation P2 (zero first external substrate) -> Python."""
    sub = dyn.substrates_snapshot.copy()
    sub[int(dyn.substrate_idx_external_exch_0[0]), dyn.compartment_extracellular_0based] = 0.0
    py = cfb.compute_bounds(
        substrates=sub,
        enzymes=dyn.enzymes_snapshot,
        cell_dry_mass=dyn.cell_dry_mass,
    ...
    mat_c = _load_perturbation_oracle("c")
    diff = _bound_diff(py, mat_c)
    assert diff["n_inf_mismatch"] == 0, diff
    assert diff["finite_max_abs_diff"] < 1e-9, diff


def test_perturbation_panel_p3_matches_oracle(m1, dyn) -> None:
    ...
    mat_d = _load_perturbation_oracle("d")
    diff = _bound_diff(py, mat_d)
    assert diff["n_inf_mismatch"] == 0, diff
    assert diff["finite_max_abs_diff"] < 1e-9, diff


def test_audit_block_in_fixture() -> None:
