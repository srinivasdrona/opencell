...
def _write_raw(tmp_path: Path, rows: list, summary_overrides: dict = None):
    ...
    (tick, e1_pool, any_complex_delta, net2_complex_delta_sum, pinched)."""
    csv_path = tmp_path / "MacromolecularComplexation_e1_lifecycle_seed000.csv"
    with open(csv_path, "w", encoding="utf-8", newline="") as fh:
        fh.write("tick,e1_pool,any_complex_delta,net2_complex_delta_sum,pinched\n")
        for tick, e1_pool, any_delta, net2_delta, pinched in rows:
            fh.write(f"{tick},{e1_pool},{any_delta},{net2_delta},{1 if pinched else 0}\n")
...
def test_build_rejects_summary_max_e1_pool_mismatch(tmp_path, monkeypatch):
    monkeypatch.setattr(hlr, "N_TICKS_MAX", 200)
    rows = [(t, 0, 0, 0, False) for t in range(1, 200)] + [(200, 0, 0, 0, True)]
    csv_path, summary_path = _write_raw(tmp_path, rows, summary_overrides={"max_e1_pool": 999})
    with pytest.raises(ValueError, match="max_e1_pool"):
        hlr.build_lifecycle_reachability_artifact(csv_path=csv_path, summary_path=summary_path)


def test_build_rejects_summary_n_net2_events_mismatch(tmp_path, monkeypatch):
    monkeypatch.setattr(hlr, "N_TICKS_MAX", 200)
    rows = [(t, 0, 0, 0, False) for t in range(1, 200)] + [(200, 0, 0, 0, True)]
    csv_path, summary_path = _write_raw(tmp_path, rows, summary_overrides={"n_net2_events": 5})
    with pytest.raises(ValueError, match="n_net2_events"):
        hlr.build_lifecycle_reachability_artifact(csv_path=csv_path, summary_path=summary_path)
...
def test_build_rejects_structurally_impossible_network2_without_e1(tmp_path, monkeypatch):
    ...
    # while e1_pool is 0 at every tick, including tick 150 -- this cannot
    # come from a real run (network 2 needs E1 > 0 by stoichiometry), so
...
def _write_raw_no_summary(tmp_path: Path, rows: list, e1_idx: int = 193, net2_idx: tuple = (23, 24)):
    ...
    killed before it could break out and write its own summary. Returns
    (csv_path, log_path)."""
    csv_path = tmp_path / "MacromolecularComplexation_e1_lifecycle_seed000.csv"
    with open(csv_path, "w", encoding="utf-8", newline="") as fh:
        fh.write("tick,e1_pool,any_complex_delta,net2_complex_delta_sum,pinched\n")
    ...
    log_path.write_text(
        "[macromol-scan] === MacromolecularComplexation (seed=0, up to 33000 ticks) ===\n"
        f"[macromol-scan] E1 local substrate index = {e1_idx}; "
        f"network-2 complex indices = [{' '.join(str(i) for i in net2_idx)}]\n"
        "[macromol-scan] tick 21000/33000 (164.4 min) max_e1=2 net2_events=26\n",
        encoding="utf-8",
    )
...
def test_committed_artifact_reports_network2_fired_naturally(artifact):
    ...
    # zero and network >= 2 actually forms a complex under the real
    # scheduler, with no conditioning -- directly falsifying the "genuine
    # biological ceiling" hypothesis for the natural lifecycle scanned.
    assert artifact["e1_ever_nonzero"] is True
    assert artifact["network2_ever_fired_naturally"] is True
    assert artifact["outcome"] == "network2_fired_naturally"
    assert artifact["probe"]["n_net2_events"] > 0
