...
def test_snapshot_ribosome_counts(inputs) -> None:
...
def test_snapshot_ribosomes_bound_matches_active(inputs) -> None:
...
def test_mrna_counts_are_sparse(inputs) -> None:
...
def test_total_aa_polymerization_invariant(inputs) -> None:
...
def test_predict_scales_linearly_with_n_active(inputs) -> None:
...
def test_predict_only_for_present_mrnas(inputs) -> None:
...
def test_predict_handles_empty_mrna_pool(inputs) -> None:
...
def test_mechanism_total_exceeds_v1_decay_balance(inputs) -> None:
...
def test_mechanism_predicts_above_doubling_requirement(inputs) -> None:
...
def test_predict_distributes_across_present_mrnas(inputs) -> None:
    ...
    expected = inputs.n_active_ribosomes * inputs.elongation_rate_aa_per_s / denom
    np.testing.assert_allclose(rate_per_copy[0], expected, rtol=1e-12)


def test_load_default_path_round_trip(inputs) -> None:
