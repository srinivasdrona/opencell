...
def _processing_events(before_monomers: np.ndarray, after_monomers: np.ndarray) -> int:
    delta = np.asarray(before_monomers, dtype=np.float64) - np.asarray(after_monomers, dtype=np.float64)
    return int(round(float(np.sum(delta))))


def _build_process(seed: int, tick: int) -> KarrProteinProcessingIIProcess:
    with runner_helpers.forbid_sut_oracle_file_io():
        process = KarrProteinProcessingIIProcess({"rng_seed": int(runner_helpers._sample_seed(seed, tick))})
...
def _run_oc_tick(
    ...
    process = _build_process(seed, tick)
    ...
    runner_helpers.overlay_observable_into_state(
        ...
        state=runtime_state,
        observable="monomers",
        vector=np.asarray(before["unprocessedMonomers"][seed, tick], dtype=np.float64),
        wids=monomer_wids,
    )
...
def _run_alpha(
    ...
    for seed in range(N_SEEDS):
        for tick in range(n_ticks):
            ...
            oc_after = _run_oc_tick(seed=seed, tick=tick, before=before, alpha=alpha)["monomers"]
...
def main() -> int:
    ...
    rows = [_run_alpha(alpha, before, karr_after_monomers) for alpha in ALPHAS]
