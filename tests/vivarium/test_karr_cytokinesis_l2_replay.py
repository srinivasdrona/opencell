...
def test_karr_cytokinesis_l2_event_replay_m5000_randstream_bound(rng_seed: int) -> None:
    ...
    with h5py.File(trace_path, "r") as trace:
        ...
        # `current_genuine_dnadamage_source()` (launcher.py main API, kept
        # as-is) returns `source_sha256_lf_normalized`/
        # `patched_sha256_lf_normalized` plus `overlay_required`, not a
        # precomputed "resolved" key -- select whichever the overlay flag
        # picks (see that function's own docstring).
        current_resolved_sha256 = (
            current_dnadamage_source["patched_sha256_lf_normalized"]
            if current_dnadamage_source["overlay_required"]
            else current_dnadamage_source["source_sha256_lf_normalized"]
        )
        assert resolved_sha256 == current_resolved_sha256, (
            f"{trace_path}: metadata.dnadamage_source_resolved_sha256={resolved_sha256!r} != "
            f"current repo resolved identity {current_resolved_sha256!r} -- "
