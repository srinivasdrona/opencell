...
# ----------------------------------------------------------------------
def test_m1_pools_schema_present_in_dynamic_mode_only():
    ...
    for sid in _KARR_DEMAND_KEYS:
        ...
        assert leaf["_default"] == proc_dyn._sub_state[
            proc_dyn._sub_id_to_idx[sid], _CYTOSOL_COMPARTMENT_0
        ]


def test_m1_pools_update_matches_internal_sub_state_after_tick():
...
def test_m1_pools_seeded_from_snapshot_at_tick_zero():
...
# ----------------------------------------------------------------------
def test_m2_throttle_unity_when_pools_abundant():
...
def test_m2_throttle_zero_when_one_pool_starved():
    proc = KarrTranscriptionProcess({"enable_throttle": True})
...
def test_m3_throttle_unity_when_pools_abundant():
...
def test_m3_throttle_partial_when_one_aa_below_demand():
...
def test_throttle_rejects_zero_and_negative_dt():
    proc = KarrTranscriptionProcess({"enable_throttle": True})
...
def test_throttle_rejects_non_finite_pool():
    proc = KarrTranscriptionProcess({"enable_throttle": True})
    pools = {ntp: 1.0e12 for ntp in _M2_CONSUMED_SUBSTRATES}
    pools["ATP"] = float("inf")
...
def test_throttle_treats_negative_pool_as_zero():
    proc = KarrTranscriptionProcess({"enable_throttle": True})
    pools = {ntp: 1.0e12 for ntp in _M2_CONSUMED_SUBSTRATES}
    pools["ATP"] = -42.0
    f = proc._compute_throttle(pools, timestep=1.0)
...
# ----------------------------------------------------------------------
def test_composer_rejects_throttle_without_dynamic_bounds():
    with pytest.raises(ValueError, match="dynamic_bounds"):
        build_karr_m1_m2_m3_engine(
            dynamic_bounds=False, enable_throttle=True,
        )


def test_throttle_off_baseline_unchanged_after_5_ticks():
    ...
    eng = build_karr_m1_m2_m3_engine(
        dynamic_bounds=True, enable_throttle=False,
    )
    eng.update(5.0)
...
def test_throttle_on_with_abundant_pools_matches_throttle_off():
    ...
    eng_on = build_karr_m1_m2_m3_engine(
        dynamic_bounds=True, enable_throttle=True,
    )
...
def test_throttle_on_with_starved_atp_freezes_m2_synthesis():
    ...
    for g, before in rna_before.items():
        assert rna_after[g] <= before + 1e-9, (
            f"gene {g} grew under f=0: {before} -> {rna_after[g]}")
    ...
    for ntp in ("ATP", "CTP", "GTP", "UTP"):
        ...
        assert sub_after[ntp] == pytest.approx(sub_before[ntp], abs=1e-9), (
            f"NTP {ntp} substrate moved under M2 f=0: "
            f"{sub_before[ntp]} -> {sub_after[ntp]}")
