...
class _LoggingMatlabRandStream:
    def __init__(self, seed: int) -> None:
        ...
        self.calls: list[tuple[str, tuple[int, ...], np.ndarray]] = []

    def rand(self, *shape: int) -> np.ndarray:
        out = self._stream.rand(*shape)
        normalized_shape = tuple(int(dim) for dim in shape)
        self.calls.append(("rand", normalized_shape, np.asarray(out, dtype=np.float64).copy()))
        return out

    def randperm(self, n: int, k: int | None = None) -> np.ndarray:
        n_i = int(n)
        if k is None:
            out = self._stream.randperm(n_i)
            normalized_shape = (n_i,)
        else:
            k_i = int(k)
            out = self._stream.randperm(n_i, k_i)
            normalized_shape = (n_i, k_i)
        self.calls.append(("randperm", normalized_shape, np.asarray(out, dtype=np.int64).copy()))
        return out
...
def test_dna_supercoiling_replay_rng_matches_matlab_stream_call_pattern_seed0() -> None:
    ...
    calls_by_tick: list[list[tuple[str, tuple[int, ...], np.ndarray]]] = []
    ...
    expected_patterns_by_tick: list[list[tuple[str, tuple[int, ...]]]] = [
        [
            ("rand", (4,)),
            ("rand", (1,)),
            ("randperm", (3,)),
    ...
    for tick, expected_patterns in enumerate(expected_patterns_by_tick):
        actual_patterns = [(method, shape) for method, shape, _ in calls_by_tick[tick]]
        assert actual_patterns == expected_patterns, (
            f"tick={tick} RNG call pattern drift: "
            f"actual={actual_patterns}, expected={expected_patterns}"
        )

    expected_stream = MatlabRandStream(0)
    expected_calls: list[tuple[str, tuple[int, ...], np.ndarray]] = [
        ("rand", (4,), np.asarray(expected_stream.rand(4), dtype=np.float64)),
        ("rand", (1,), np.asarray(expected_stream.rand(1), dtype=np.float64)),
        ("randperm", (3,), np.asarray(expected_stream.randperm(3), dtype=np.int64)),
        ("randperm", (3,), np.asarray(expected_stream.randperm(3), dtype=np.int64)),
        ("rand", (), np.asarray(expected_stream.rand(), dtype=np.float64)),
        ("rand", (), np.asarray(expected_stream.rand(), dtype=np.float64)),
        ("randperm", (3,), np.asarray(expected_stream.randperm(3), dtype=np.int64)),
        ("randperm", (3,), np.asarray(expected_stream.randperm(3), dtype=np.int64)),
        ("rand", (), np.asarray(expected_stream.rand(), dtype=np.float64)),
    ...
    for idx, (actual, expected) in enumerate(zip(actual_calls, expected_calls)):
        actual_method, actual_shape, actual_values = actual
        expected_method, expected_shape, expected_values = expected
        assert actual_method == expected_method, (
            f"call={idx} method drift: actual={actual_method}, expected={expected_method}"
        )
