...
def _replication_tick_state(*, tick: int) -> dict[str, object]:
    oracle = runner_helpers.load_karr_oracle("Replication")
    process = runner_helpers._replication_process(0)
    return {
        "substrate_wids": [*process.dntp_wids, process.atp_wid],
...
def test_replication_primary_fixture_is_legitimate_noop() -> None:
    oracle = runner_helpers.load_karr_oracle("Replication")
    ...
    assert runner_helpers._REPLICATION_ORACLE_PATH.name == "Replication.npz"


def test_replication_tick_ignores_trace_hint_bypass_payload() -> None:
    honest_state = _replication_tick_state(tick=0)
    ...
    runner_helpers._replication_process.cache_clear()
    honest = runner_helpers._run_replication_tick(0, 0, honest_state)
    runner_helpers._replication_process.cache_clear()
    cheated = runner_helpers._run_replication_tick(0, 0, cheated_state)
...
def test_replication_constant_zero_primary_channel_fails(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    payload = runner.run_design_a(
        ...
        seeds=[0, 1, 2],
        m_ticks=5,
        out_dir=tmp_path / "Replication_zero_primary",
        bootstrap_B=10,
    )
...
def test_replication_primary_exact_match_is_legitimate_noop(tmp_path: Path) -> None:
    payload = runner.run_design_a(
        process="Replication",
        seeds=[0, 1, 2],
        m_ticks=5,
        out_dir=tmp_path / "Replication_legitimate_noop",
        bootstrap_B=10,
    )
