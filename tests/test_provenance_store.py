...
def _measured(
    store, *, name="Vmax_PGI", value=1.5, unit="mM/s", source="doi:10.1093/bioinformatics/18.4.516"
):
...
def test_record_and_read_back(store) -> None:
    ev = _measured(store)
...
def test_event_id_is_content_addressed(store) -> None:
    ev1 = _measured(store)
...
def test_jsonl_is_human_readable(tmp_path) -> None:
...
def test_supersedes_resolves_current(store) -> None:
...
def test_tuning_within_range_ok(store) -> None:
...
def test_tuning_outside_range_rejected(store) -> None:
    base = _measured(store, value=1.5)
...
def test_query_by_scope(store) -> None:
...
def test_corrections_are_new_events_not_mutations(tmp_path) -> None:
