...
def test_transcription_enrolled_in_allocator_and_10_tick_nonnegative() -> None:
    ...
    tx_proc = composite["processes"]["karr_transcription"]

    assert "karr_transcription" in consumers
    assert consumers["karr_transcription"] == list(tx_proc.allocation_substrate_wids)
    assert set(consumers["karr_transcription"]) == {"ATP", "CTP", "GTP", "UTP"}

    tx_topology = composite["topology"]["karr_transcription"]
    assert tx_topology.get("substrates_allocated") == ("substrates_allocated",)
