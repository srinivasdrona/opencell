...
def test_metabolism_enrolled_in_allocator_and_10_tick_nonnegative() -> None:
    ...
    m1_proc = composite["processes"]["karr_metabolism"]

    assert "karr_metabolism" in consumers
    assert consumers["karr_metabolism"] == list(m1_proc.allocation_substrate_wids)
    assert consumers["karr_metabolism"], "Metabolism allocator vector must not be empty"

    m1_topology = composite["topology"]["karr_metabolism"]
    assert m1_topology.get("substrates_allocated") == ("substrates_allocated",)
