...
def _approved(**overrides) -> ParameterCard:
    ...
    defaults = dict(
        ...
        cross_references=[{"source_doi": "10.1093/nar/alt", "value": 105.0,
                           "unit": "1/s", "agrees": True, "note": "close"}],
...
def _has_issue(issues: list[ValidationIssue], field: str,
               severity: Severity | None = None) -> bool:
    """Check whether a specific field appears in the issue list."""
    for iss in issues:
        if iss.field == field:
            if severity is None or iss.severity is severity:
                return True
    return False
...
class TestConstruction:
    ...
    def test_approved_card_has_all_context(self):
        card = _approved()
...
class TestStatusRoundTrip:
    ...
    def test_dict_round_trip(self, status):
...
class TestValueRangeValidator:
    ...
    def test_inverted_uncertainty_bounds(self):
...
class TestTransformationValidator:
    def test_original_value_without_transformation_warns(self):
    ...
    def test_original_value_without_unit_warns(self):
    ...
    def test_documented_transformation_ok(self):
        card = _draft(original_value=110.0, original_unit="s^-1",
                      transformation="identity (same unit)")
        issues = validate_card(card)
...
class TestSerialisation:
    ...
    def test_yaml_preserves_cross_references(self, tmp_path: Path):
        card = _approved()
...
class TestAudit:
    ...
    def test_cross_ref_coverage(self):
        cards = [
    ...
    def test_gate_acknowledged(self):
        card = _draft(used_in_gate_tests=True, gate_acknowledged=True,
                      acknowledgement_reason="only param available")
        report = audit_parameters([card])
        assert card.parameter_id in report.gate_acknowledged
        assert card.parameter_id not in report.gate_violations

    def test_approved_in_gate_ok(self):
    ...
    def test_validation_issues_collected(self):
...
class TestCIGateCheck:
    def test_passes_all_approved(self):
        cards = [_approved(parameter_id=f"p{i}") for i in range(2)]
        ok, msg = ci_gate_check(cards)
        assert ok is True
        assert "PASS" in msg

    def test_fails_draft_in_gate(self):
        card = _draft(used_in_gate_tests=True)
        ok, msg = ci_gate_check([card])
        assert ok is False
        assert "DRAFT" in msg

    def test_passes_draft_acknowledged(self):
        card = _draft(used_in_gate_tests=True, gate_acknowledged=True,
                      acknowledgement_reason="testing")
        ok, msg = ci_gate_check([card])
        assert ok is True

    def test_warns_reviewed_in_gate(self):
        card = _reviewed(used_in_gate_tests=True)
        ok, msg = ci_gate_check([card])
    ...
    def test_fails_approved_with_errors(self):
    ...
    def test_fails_missing_unit(self):
        card = _draft(unit="")
        ok, msg = ci_gate_check([card])
        assert ok is False
        assert "required fields" in msg.lower()

    def test_passes_no_gate_usage(self):
...
class TestEdgeCases:
    ...
    def test_full_lifecycle_draft_to_approved(self):
        ...
        card.cross_references = [{"source_doi": "10.1234/x", "value": 100.0,
                                   "unit": "1/s", "agrees": True, "note": ""}]
        issues = validate_card(card)
