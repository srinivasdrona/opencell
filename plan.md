...
- **DNARepair strict rubric = honest COINCIDENTAL** (not faked GENUINE): `test_l2_1_strict_rubric.py::_classify()` overlays only standard observables and does NOT inject the chromosome hidden-read surface, so DNARepair can't fire its chromosome-coupled reactions there. Pin updated to COINCIDENTAL with inline reason. **Residual/follow-up:** extend the strict-rubric classifier to inject `hidden_read_surface` (chromosome) for such processes so they can be scored on real biology (affects any chromosome-coupled process, not just DNARepair).
...
| 3 | **L2.1** bit-identity replay | ✅ **both blockers closed (2026-07-13)**; legacy 26 pass / 2 skip (RibosomeAssembly + RNAModification no-op traces), strict rubric 28/28 | merge `agent/dnarepair-replay-channel`; then optional: strict-rubric chromosome injection for hidden-read processes | — |
