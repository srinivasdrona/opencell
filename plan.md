...
- **Gate verdict: PASS (diverge_cells=0)** after the Translation fix — vocab 28/28 CONFORM (ordered), stoich 5 CONFORM + 23 N/A (signed), state-usage + constants INFO. Self-tests: `tests/integration/test_gate2_oc_vs_spec_gate.py` (4 pass). NOT yet CI-wired (option: wire blocking on the reliable classes = ordered vocab + signed stoich).
