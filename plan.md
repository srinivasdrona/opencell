...
**Live processes / agents (2026-07-09 ~01:15 IST):** none running. **✅ WSL RESTORED.** The distro rootfs had been reset (uptime showed 1 min; only python3.10 remained, python3.12 gone), which orphaned the venv (base interpreter `/usr/bin/python3.12` missing). Fix: `add-apt-repository ppa:deadsnakes/ppa` + `apt install python3.12 python3.12-venv python3.12-dev` (got 3.12.13, exact match to the venv's original). All site-packages had persisted on `E:` (jax/numpy/pint/roadrunner intact), so reinstalling only the interpreter revived the venv — no venv rebuild needed. Verified: roadrunner 2.9.2 / jax 0.10.0 / numpy 2.4.4 import; editable `opencell` imports; both L1b gates green; 19 L1b tests pass. **If WSL rootfs resets again, the recovery is just the deadsnakes python3.12 reinstall above.**
...
**➡️ NEXT: L2 family = 6 sub-gates.** L2.0 (static schema — ✅ GATE DONE, 28/28 GREEN, CI-wired), L2.0a (allocator input — NOT built), L2.1 (per-process bit-identity replay — ~44/46 strict last known + flagged S3 DNARepair regress), L2.2 (distributional — 2/7 DEEP landed, partial), L2.4 (chassis conservation — NOT built), L2.5 (composition — PAUSED). Critical path: L2.0→L2.0a→L2.1→L2.2→L2.4→L2.5. **Current step (2026-07-09): L2.0 gate formalized + green; next candidates are L2.0a build, or the flagged L2.1 S3 DNARepair regression.**
