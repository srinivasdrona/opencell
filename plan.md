...
**Live processes / agents (2026-08-05, worktree `E:\opencell-worktrees\l22-macromol-closure-20260805`, branch `agent/l22-macromol-closure-20260805`):** **RESOLVED — no live processes.** The full-cycle MATLAB probe (`scripts/matlab/full_cycle_event_scan_macromol.m`, seed 0, real Karr per-tick scheduler, no conditioning) was deliberately stopped by the operator at real simulated tick 24,300/33,000 once evidence was decisive: E1 (`MG_429_MONOMER`) pool leaves zero at tick 5,892, reaches the stoichiometric max of 2 by ~tick 8,100, and network≥2 fires **27 real complex-formation events** (first at tick 8,166), each independently verified against the raw CSV to co-occur with `e1_pool==2`. This falsifies the "genuine biological ceiling" hypothesis and confirms the "M=100/tick_offset=0 sampling-window artifact" hypothesis (same class as RibosomeAssembly's tick~238 fix). Natural cell division (`Geometry.pinched`) was NOT reached — this is honest, disclosed partial coverage (`stop_reason=operator_stopped_after_decisive_evidence`, `partial_coverage=true`), not a claim of full-cycle completion. New NON-GATING evidence artifact generated + self-validates: `docs/phase_f/l2_2_design_a/h12/lifecycle_reachability/MacromolecularComplexation_h12_lifecycle_reachability.json` (generator: `scripts/l22_evidence/h12_lifecycle_reachability.py`, tests: `tests/scripts/test_h12_lifecycle_reachability.py`, 150 total tests passing across both `test_h12_lifecycle_reachability.py` + `test_h12_condition_gated.py`, condition-gated `lifecycle_reachability_status` lock verified still intact/untouched). §5 addendum written to `MACROMOLECULARCOMPLEXATION_NETWORK2_E1_PROVENANCE.md`. `evidence_index.json`/`PROCESS_CATALOG.yaml` intentionally untouched (shared file / already correct). SENTINEL_FAIL itself remains open by design — network 2 is Monte Carlo by construction (`buildProteinComplexs_montecarlokinetic`), so `H12_CONFIRMED` is structurally inapplicable regardless of reachability; this is expected, not a bug. MATLAB **UNLOCKED** (probe stopped). WSL up (`.venv-wsl`).
+
