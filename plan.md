...
**Live state (2026-07-23 ~00:45 IST) — HONESTY RE-BASELINE (supersedes L2.2 claims below):**
- 🛑 **The L2.2 "green" layer is largely hollow — DO NOT trust pinned L2.2 verdicts.** `tests/vivarium/test_l2_2_strict_rubric.py` only checks the pin == a hardcoded `EMPIRICAL_VERDICTS` dict in `scripts/probe_l2_2_strict_audit.py` (tautology; no live run). `tmp/l2_2_audit/` is empty (no surviving evidence). Live re-run 2026-07-23 via `tests/vivarium/l2_2_design_a_runner.py` (50 seeds × 10 ticks): **ProteinDecay = FAIL** (substrates W1=2.09 vs threshold 1.0, ≥30 samples both sides); **RNAModification = runner-unsupported** (its VERIFIED_GENUINE was never obtainable). The test docstring itself says *"Real upper bound on honest L2.2 PASSes: 4 of 22."* → **the "truly green" process count is UNKNOWN** pending a live L2.2 re-baseline.
- 📄 **New artifact:** `docs/phase_f/LADDER_TRUE_STATE_2026-07-22.md` — honest per-rung + per-process state, with a 2026-07-23 correction banner folding in the hollow-L2.2 finding. Flags 3 STALE "source of truth" trackers (`docs/phase_e/L2_STATUS.md` 2026-05-30; `docs/phase_f/l2_2_design_a/L2_2_GATE_TRACKER.md` 2026-06-06; `docs/phase_f/L2_5_PAIR_TRACKER.md` Day-33) + ~43 abandoned toy-cell fossil todos.
- ⚠️ **ProteinDecay FAIL — DISAMBIGUATED 2026-07-23:** re-ran the identical runner on `main` (pre-uncap) → **byte-identical FAIL** (substrates W1=2.086792). So it is **NOT an uncap regression**; the pin was always disconnected from the runner (`karr_protein_decay*.py` is unchanged main-vs-branch). Separately, `KARR_SINGLE_SEED_REUSED` observed → L2.2 may compare OC's ensemble vs a single Karr point, undermining the distributional premise itself.
- 🔬 **ChromCond (the only L2.1 FAIL):** rechecked — NOT a 30-line filter fix. The reverted 354-line `getAccessibleRegions` port (`tmp/FAILED_chromcond_fix_520k.diff`) already implemented monomer+complex exclusion and STILL failed. `381ea0e` WIP (RNG+geometry correct, over-binds +1–3/tick) remains FAIL. Residual = accessibility count-drift → RNG desync. Multi-hour port + debug; deferred.
- **Worktrees:** clean (only main checkout registered; `git worktree prune` no-op; `E:\opencell-worktrees` absent; `E:\opencell-mirrors\{wcEcoli,WholeCell}` are load-bearing Karr source refs — keep).
- **Next honest move:** live L2.2 re-baseline (`l2_2_design_a_runner.py` across `SUPPORTED_PROCESSES`, recording Karr seed-count), rebuild the pin from real results. Azure-independent. Until then, treat NO process as "L2.2 validated."

**Live state (2026-07-19 ~02:45 IST) — (L2.4/uncap work; still valid, but its "L2.1/L2.2 strict 50 passed" line is exactly the hollow-pin claim RE-BASELINED above):** transcription-v3 fix LANDED — **A1 uncap unblocked at the chassis level**; L2.4 design HARDENED; ready to build the L2.4 gate.
...
- ✅ **L2.4 Part A BUILT + GREEN on the uncapped branch** (codex `593038b`+`2a19e35`, `scripts/l2_4_verify_conservation.py`). Integer-exact per-WID conservation on non-exchange substrates: **PASS, 100 ticks × 4 seeds, max_abs_unattributed=0** (integer arithmetic), no STABILITY_FAIL, 124 exchange WIDs excluded. Verified non-vacuous: planted-leak self-test returns CONSERVATION_FAIL via the real `evaluate_tick_part_a`.
- ✅ **L2.4 Part B BUILT + GREEN** (codex `c2a913d`+`31664c6`; off-by-one FIX `2ecf97c`). Adds consumption ≤ allocation + nonnegative-pool checks. Codex's first verdict was a FALSE `PARTB_FAIL` (307 over_allocation): it snapshotted `substrates_allocated` AFTER `engine.update()` (= allocator's recomputed NEXT-tick allocation) and compared to THIS-tick consumption. Diagnosed via probe (protein_folding reads alloc 2454, consumes 1798 clipping to floor(alloc); post-update snapshot was next-tick's 656). Fixed by snapshotting BEFORE the update. **Full gate (A+B) now PASS on the uncapped branch, 100t×4s**; 6 self-tests green incl. a 14-tick regression guard the 1-tick smoke couldn't catch.
- **So L2.4 (A+B) CERTIFIES the A1-uncap chassis: conserves mass integer-exactly + every consumer honors consumption ≤ allocation + no negative pools, on the uncapped branch (D8 verdict target).**
- **Next move when resuming:** (a) push the branch (unpushed: `3fe4bef`→`2ecf97c`), confirm-before-push; (b) optionally Batch 3 = L2.4 mechanical write-surface audit (D6-rev) + exchange shadow audit (D2-rev) for full-spec completeness; (c) with L2.4 green, the A1 uncap is cleared to land — decide merge-to-main path.
...
| 2 | **L2.0a** allocator input | ✅ **GATE BUILT + baseline resolved (2026-07-14).** Oracle extracted (`scripts/matlab/extract_l2_0a_allocator_oracle.m`, 28 procs × 4332 slots); gate `scripts/probe_l2_0a_allocator_input.py` + 5 anti-cheat tests on branch `agent/l2-0a-gate`. Honest RED baseline (111 divergences = all A1 oversupply-cap fork) → **A1 fix prepared** (`agent/l2-0a-uncap` @ `adf2d1a`) flips it **403/403 GREEN**. | land A1 fix after L2.4 (chassis integer-count); merge gate branch | MATLAB (done) |
...
**🔬 EVENT-CLASS L2.2 phase-0 findings (2026-07-15).** The 4 `event_class` (harness_type) procs reconciled against disk + Karr source:
- **FtsZPolymerization** — GENUINE: its standard 100-tick trace has REAL events (substrates 60/100 ticks, enzymes 100/100); `test_karr_ftsz_polymerization_l2_replay.py` bit-identity PASS. Strict-rubric label `NOT_WIRED` is **STALE** — credit it.
- **RibosomeAssembly** — GENUINE via event-window: standard window no-op (253 events/cycle land elsewhere), `test_karr_ribosome_assembly_l2_replay.py::test_karr_ribosome_assembly_l2_event_replay` PASS on the `tick_offset` burn-in trace. Strict-rubric `UNVALIDATABLE_EVENT_CLASS` is **STALE** — credit it.
- **DNADamage** — was VACUOUS (standard trace 0 mutated ticks = mutual-no-op pass). Phase-0: FIXABLE. Karr `DNADamage.m:94` rate table → ~8 spontaneous events/cycle (base deamination 0.1 + base loss 7.8; UV-B needs radiation absent from sim). Sparse (~1/4000 ticks) → 100-tick window catches ~0 = sampling artifact, same as RibosomeAssembly. Fix: full-cycle scan to locate the ~8 firing ticks → event-window trace → replay harness (MATLAB=Copilot, Python=codex).
- **Cytokinesis** — was VACUOUS. Phase-0: BLOCKED for isolation. `Cytokinesis.m:173-175` `evolveState` returns unless `chromosome.segregated` (true only at end-of-cycle) → the 0-events-in-32400 isolated scan is CORRECT, not a miss. **Correction (2026-07-15): NOT defer-to-L2.4.** L2.4 is a ≤100-tick, NO-oracle conservation gate — Cytokinesis is idle for those 100 ticks (pre-segregation) so it trivially "conserves" (0==0), giving zero division-fidelity signal. Cytokinesis's division event is a full-cell-cycle **phenotype → L5** (~30K-tick ensemble: does the cell divide, on time, with correct FtsZ dynamics). The only pre-L5 shortcut is a Karr-state-conditioned one-tick probe at the division window (v4 L2.event) — but that STILL needs a full-cycle Karr division-window trace (same prerequisite). No isolated oracle can honestly exist.
- **Net:** honest event-class picture is 2 genuinely validated (FtsZ, RibosomeAssembly — scoreboard stale), 1 fixable (DNADamage), 1 chassis-deferred (Cytokinesis). `test_l2_2_strict_rubric.py` EXPECTED_L2_2_VERDICTS needs updating once FtsZ/RA credited + DNADamage closed.
...
| **A1** | OC allocator caps scale at `min(1.0, counts/total_demand)`; Karr's `tmp = counts/max(1, sum_req)` can be >1 (over-allocates surplus). | ✅ **ROOT-CAUSED + FIX PREPARED (2026-07-14, branch `agent/l2-0a-uncap` @ `adf2d1a`, NOT landed).** Cap removed → allocator bit-matches `evolveState.m:36-37`. Audited all 24 consumers (`docs/phase_f/A1_ALLOCATOR_UNCAP_CONSUMPTION_AUDIT.md`): AMOUNT=0, every consumer enforces consumption≤allocation ⇒ pool can't go negative. Gate-verified: L2.0a 403/403 GREEN, L2.1 28/28, L2.2-strict, allocator units reconciled. **Hold:** uncapping de-integerizes the shared pool via the v6 chassis's scope-reduced **v3** transcription (fractional `total_nt/4·dt`), tripping translation's integer check in ~24 integration tests → chassis integer-count integrity, whose gate is **L2.4 (NOT BUILT)**. Land after L2.4. | — |
...
| **A3b** | OC's metabolism: LP solves with bounds from `_sub_state` (full pool), writeback computes stoichiometrically-consistent delta, then **only consumption entries are capped to allocation** (production entries untouched). Mass conjured per tick. | ⚠️⚠️ | Day-13 ATP linear drain |
| **A3** | OC's metabolism LP bounds derive from `_sub_state` (pool tracker), not from allocator allocation. Karr's LP gets bounds from `mod.substrates = allocation`. LP feasible regions differ in chassis context. | ⚠️ | Setup for A3b |
| **A1** | OC allocator caps scale at `min(1.0, counts/total_demand)`; Karr's `tmp = counts/max(1, sum_req)` can be >1 (over-allocates surplus). | ✅ **ROOT-CAUSED + FIX PREPARED (2026-07-14, `agent/l2-0a-uncap` @ `adf2d1a`, NOT landed)** — see A1 detail in the runway bug table above; held pending L2.4. | — |
| A2 | Karr randomizes process order per tick via `randStream.randperm`; OC uses Vivarium topological deterministic order. | Benign for mass balance (order-independent end state) | — |
