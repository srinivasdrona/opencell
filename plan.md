...
**Live processes / agents (2026-07-10 ~01:50 IST — PARKED for the night, resume tomorrow WITH the MATLAB license fix):** none running. WSL up (deadsnakes python3.12 reinstall recovery if the distro rootfs resets again). **✅ JAX FULLY EXCISED** (`83bb93c`, pushed). **✅ ROOT CLEANED** (`7cdc7ce`, pushed): durable status docs live under `docs/archive/status/`, NOT repo root; root `/STATUS_*.md` + `/PROMPT_*.md` are gitignored scratch.

**🛑 6 UNPUSHED COMMITS on main (holding for operator confirm):** `bfb7cc2` (stoich design v2), `b2e380d` (derived input spec freeze), `1f3a37b`+`4f0dd51` (spec annotation fixes), `66e01e4` (input taxonomy discovery), `284a284` (plan). Ask before pushing.

**➡️ TOMORROW'S FIRST MOVES (in order):**
1. **Renew MATLAB license** (MathWorks Error 10, expired). Unblocks: L2.0a build, L2.1 DNARepair-R-M + DNASupercoiling-seed_1, L2.4 probe — AND the new Gate-0 audit + resolving the 12 KB_COMPUTED substrate lists below.
2. **Settle the PARKED input-fidelity scope decisions** (all analysis done, just needs ratify + build):
   - **Frozen spec extension:** add the 4 categories the fixture-only spec MISSES — state-object refs, `fittedConstantNames`, `fixedConstantNames` (+ options recorded-not-all-gated). Source: `docs/phase_f/KARR_INPUT_TAXONOMY.md` / `karr_input_taxonomy.json` (`66e01e4`).
   - **Options gating rule (RATIFIED direction, mechanical not hand-tagged):** record ALL options in the spec, but Gate-2 checks only `optionNames − {stepSizeSec, verbosity, seed}` (subclass-added = material, e.g. Metabolism's `linearProgrammingOptions/tolerance/realmax/macromoleculeStateInitialization`). `stepSizeSec` asserted `==1` globally (not dropped); `seed` never value-checked; `verbosity` ignored. Spec marks `gated: true/false` per field (avoid false-confidence). Verify tomorrow that base-class options are truly the universal set.
   - **12 KB_COMPUTED substrate vocabs:** decide resolve-via-`WholeCellKB`-on-disk vs accept-fixture-as-resolved-authority.
3. **Gate 0 (one-time): fixture ⟺ MATLAB `.m` source completeness audit.** The whole POINT of the frozen spec is to catch a future port silently missing a sub-module; but the spec is currently anchored to the FIXTURE (an extract), not the source — so a fixture-extraction omission would freeze silently. Audit each process's substrate/enzyme/state-ref sets in the fixture against the real `.m` (taxonomy done; ~12 need KB resolution). Karr won't drift → one-time, not standing.
4. **Then build Gate 2:** uniform input-accessor contract on OC processes (they expose inputs under inconsistent attrs: `substrate_wids`/`_substrate_wids`/`allocation_substrate_wids`/`gtpase_wids`), then check OC ⟺ frozen spec. **Known real bug to fix: Transcription loads 4/12 substrates** (missing ADP/AMP/CMP/GMP/UMP/PPI/H2O/H). Adjudicate TerminalOrganelleAssembly (compartment-tag representation), Metabolism (585 FBA metabolites via allocation path), Translation (26, no exposed vocab).
5. **Gate 1 CI freeze** (short, anytime): wire `spec==fixture` + hash-lock + determinism as a blocking CI job.
...
- **Gate 2 (OC ⟺ frozen spec): probe built, NOT yet a gate.** Compares each OC process's loaded input vocabulary to the frozen spec. Baseline: 24/28 conform; real bugs = **Transcription (loads 4/12 substrates, missing ADP/AMP/CMP/GMP/UMP/PPI/H2O/H)**; adjudicate TerminalOrganelleAssembly (compartment-tag representation 16 vs 8), Metabolism (585 FBA metabolites via allocation path), Translation (26 substrates, no exposed vocab). Needs a UNIFORM input-accessor contract on processes (they expose inputs under inconsistent attrs: `substrate_wids`/`_substrate_wids`/`allocation_substrate_wids`/`gtpase_wids`) so Gate 2 checks robustly. **Then fix Transcription + adjudicate the 3.**
...
**L2 sub-gates (background):** L2.0 (schema — ✅ 28/28 GREEN, CI-wired), L2.0a (allocator input — designed, needs MATLAB extraction), L2.1 (bit-identity — 46/2/3; DNARepair R-M + DNASupercoiling seed_1 both blocked on expired MATLAB license), L2.2 (distributional — partial), L2.4 (chassis — designed), L2.5 (composition — PAUSED). **🛑 MATLAB license EXPIRED** — blocks L2.0a build + both L2.1 failures + L2.4 probe; operator renewing.
...
| 3 | **L2.1** bit-identity replay | built; ~44/46 strict + S3 DNARepair regress | RUN 28 replays → true baseline; fix S3 regress (`l2-regress-dnarepair-replay-s3`) | — |
