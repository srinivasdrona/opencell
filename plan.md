...
1. **Gate 1 CI freeze** (short): wire `spec==fixture` + MANIFEST hash-lock + determinism as a blocking CI job. Now safe — spec is source-anchored on vocab+stoich+constants.
2. **Gate 2** (OC ⟺ frozen spec): uniform input-accessor contract on OC processes (inconsistent attrs `substrate_wids`/`_substrate_wids`/`allocation_substrate_wids`/`gtpase_wids`), then check OC-loaded inputs vs spec for all 28. **Known real bug to fix: Transcription loads 4/12 substrates** (missing ADP/AMP/CMP/GMP/UMP/PPI/H2O/H — likely needs stoichiometry wiring, not just declaration). Adjudicate TerminalOrganelleAssembly (compartment-tag repr), Metabolism (585 FBA via allocation), Translation (26, no exposed vocab).
3. **Spec extension** (the 4 fixture-only-blind categories): state-object refs (already in `_gate0_source_truth.json`), fixed/fitted constants, options with the mechanical gating rule below. Now that Gate 0 verified all these vs source, extending the spec is low-risk.
