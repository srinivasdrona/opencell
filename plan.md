...
**Live state (2026-07-15 ~10:00 IST):** ⚙️ **RUNNING: MATLAB DNADamage full-cycle extraction** (PID 17588, `scripts/matlab/run_dnadamage_fullcycle.m` → `extract_per_process_traces_v2({'DNADamage'},'dnadamage_fullcycle',32400,uint32(0),0)`, ~1.5–2.5 h, logs `dnadamage_extract{,_err}.log`, output `data/m1_sources/karr_native/dnadamage_fullcycle/DNADamage_32400ticks.mat`). Captures chromosome damage channel (damagedBases/abasicSites/…) to locate DNADamage's ~8 sparse spontaneous events for L2.event-window replay. A blocking wait shell (shellId `wait-dnadamage`) will notify on exit; then inspect firing ticks → delegate Python event-window harness to codex. MATLAB UNLOCKED; WSL up (`.venv-wsl`).
