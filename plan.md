...
**Half B — loop started, HB1 partial.** The 3-role loop (Opus planner / gpt-5.3-codex doer / Sonnet 5 checker) runs the 6-phase Half B queue (`hb-1`..`hb-6` todos). HB1 (`4f3af71`, extract exhaustive Karr stoichiometry oracle) came back **partial: 6/28 processes extracted (Metabolism, DNADamage, DNARepair, RNAModification, tRNAAminoacylation, ProteinModification), 22 BLOCKER, 875 entries.** Generator: `scripts/extract_karr_stoichiometry.py`; output: `data/karr_method_inventory/karr_stoichiometry/`.

**🔧 HB1 re-run scoping (from full STATUS — the 22 blockers split into TWO classes, NOT one):**
1. **Matrix-in-KB** — process has a `reactionStoichiometryMatrix` but it's built in `.m` `initializeConstants` from WholeCellKB, not in the flat fixture (e.g. the gene-expression/protein processes). Fix: extract from `.m`/WholeCellKB.
2. **No-matrix / inline-arithmetic** — process has NO stoichiometry matrix at all; it does direct substrate arithmetic inside `evolveState` (spot-checked: `DNASupercoiling.m:495-499` does ATP/H2O→ADP/PI/H inline). Fix: the oracle for these must be extracted from the `evolveState` consume/produce logic itself, not a matrix lookup.
The naive "point codex at a different fixture field" is WRONG for class 2. HB1 re-run must handle both: matrix extraction where a matrix exists, inline-stoichiometry extraction from `evolveState` where it doesn't.
