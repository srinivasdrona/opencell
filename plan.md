...
- ✅ **L2.4 Part A BUILT + GREEN on the uncapped branch** (codex `593038b`+`2a19e35`, `scripts/l2_4_verify_conservation.py`). Integer-exact per-WID conservation on non-exchange substrates: **PASS, 100 ticks × 4 seeds, max_abs_unattributed=0** (integer arithmetic), no STABILITY_FAIL, 124 exchange WIDs excluded. Verified non-vacuous: planted-leak self-test (`tests/integration/test_l2_4_conservation_gate.py`) returns CONSERVATION_FAIL via the real `evaluate_tick_part_a`; independently reproduced (20t×2s PASS). Integer-validity check (`_integer_or_failure`, fractional>1e-9 → FAIL) would have caught the pre-fix fractional NTP. So Part A CERTIFIES the A1-uncap chassis conserves + is integer-clean.
- **Next move when resuming:** build **L2.4 Part B** (Batch 2) — consumption ≤ allocation (D8 Part B), mechanical write-surface snapshot audit (D6-rev), exchange-WID shadow audit (D2-rev), + a planted-fractional-delta self-test (Part A's `fractional` path exists but is untested). Then full L2.4 (A+B) green → uncap can land. Confirm-before-push the branch (unpushed: `3fe4bef`→`2a19e35`).
