...
**➡️ NEXT (operator priority 2026-07-09): STOICHIOMETRY FIDELITY GATE — "the real biology."** Design v2 at `docs/phase_f/STOICHIOMETRY_FIDELITY_GATE.md` (gpt-5.4-reviewed: v1 NEEDS-REWORK → v2 fixes all 4 blocking findings — role=both/compartment keying, none-class N/A, coefficient check dropped as unsound→L2.1, typed+ratcheted known_deviations). Static, MATLAB-free (oracle JSONs on disk). Builds as a new L1b check `check_stoichiometry_coverage`. Deferred: README/CI anti-laundering honesty (todo `readme-anti-laundering-honesty` — detector real+tested but not CI-enforced, mixed enforcement).
