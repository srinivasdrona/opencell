...
**Live processes / agents (2026-08-05, worktree `E:\opencell-worktrees\l22-macromol-closure-20260805`, branch `agent/l22-macromol-closure-20260805`):** **DONE — no live processes.** Summary of the full arc: a first full-cycle MATLAB probe (seed 0, real `sim.evolveState()` scheduler) ran to tick ~23,734 and was stopped; while finalizing its artifact, a genuine CSV-logging defect was found (write-gate `any_complex_delta_total = sum(d(:))` — a signed sum across every complex species — can cancel to exactly 0 and silently drop a genuine net2-relevant tick's row when an unrelated complex changed oppositely in the same tick). **Fixed** by adding a cancellation-safe `any_complex_changed = any(d(:) ~= 0)` boolean column, used for both the CSV write-gate and the `n_any_complex_events` counter (`any_complex_delta_total` kept informational-only, never a gate again); `_recompute_from_csv` now requires and reads this column, fail-closed if absent. That first run's raw CSV/log were discarded as unusable (real, if bounded, undercount risk) and a **second, full run from tick 1** was launched with the corrected script (cannot resume mid-run: the real scheduler's RNG stream is only reproducible from cell birth) — this is the run whose numbers are final below.

**Final measured evidence (second, corrected run, seed 0, ticks 1–18,300 of a 33,000-tick ceiling / ~32,400-tick estimated natural cycle — operator-stopped once decisive, ~56% partial coverage, NOT natural division):**
- E1 (`MG_429_MONOMER`, `e1_monomer_count_direct_state_read`) remains exactly 0 through tick 8,263; first nonzero at tick **8,264**; max observed value **2** (the exact 2-copies-per-pentamer stoichiometric requirement).
- Per-complex **formation-only** event counts (`n_net2_formation_events_by_complex`, sign-agnostic degradation excluded): `MG_041_062_429_PENTAMER` = **9** (first formation tick 8,386), `MG_041_069_429_PENTAMER` = **3** (first formation tick 8,569). Both complexes fired independently → `network2_competition_status = "both_complexes_fired"` (load-bearing 2-way competition evidence for this seed, not a degenerate single-candidate draw).
- This falsifies the "E1 fixture-constant-zero is a genuine biological ceiling" hypothesis for the accepted 50-seed × 100-tick natural census and confirms the sampling-window-artifact hypothesis (same class of effect as `RibosomeAssembly`'s tick~238 activation). An earlier, scheduler-flawed prototype had observed E1 leaving zero much earlier (~tick 5,892); that figure is superseded and disclosed in the provenance doc only as a record that scheduler-ordering fidelity (finding #2: real `sim.evolveState()`, seeded `randStream.randperm` process order, tRNAAminoacylation-before-Translation rejection loop) measurably changes this process's dynamics — tick 8,264 is the number that stands.
- **Static-source vs. empirical claims kept separate (finding #6):** `H12_CONFIRMED` is structurally inapplicable to this unit because `buildProteinComplexs_montecarlokinetic` draws `randStream.rand()` every iteration regardless of competition (a source fact, independent of any run). This run's empirical contribution is narrower and additive: it shows the Monte Carlo branch is not just theoretically reachable but actually, repeatedly exercised by both complex identities for this one seed — it does not itself prove `H12_CONFIRMED`-inapplicability (that was already true by source inspection) and does not generalize to all 50 seeds.

**Artifacts finalized and self-validating:** `docs/phase_f/l2_2_design_a/h12/MACROMOLECULARCOMPLEXATION_NETWORK2_E1_PROVENANCE.md` §5 rewritten with the numbers above and corrected mechanism/scope wording; `docs/phase_f/l2_2_design_a/h12/lifecycle_reachability/MacromolecularComplexation_h12_lifecycle_reachability.json` regenerated against the rewritten doc (self-validates); the pre-existing `docs/phase_f/l2_2_design_a/h12/condition_gated/MacromolecularComplexation_h12_condition_gated.json` was ALSO regenerated (finding #1) so its `e1_provenance_ref_sha256_lf_normalized` matches the same rewritten doc — diffed to confirm ONLY the hash + `generated_at` timestamp changed, `lifecycle_reachability_status` remains pinned `"UNRESOLVED"` as required. Full suite `tests/scripts/test_h12_lifecycle_reachability.py` + `test_h12_condition_gated.py`: **156/156 passing**, 0 expected failures (the prior 5 staleness failures are now resolved by the regeneration above). Ruff clean on all modified Python files.

`evidence_index.json`/`PROCESS_CATALOG.yaml`: catalog's `closed_form_dominant` withdrawn to `candidate` via `decisions/dec-004-macromol-network2-closed-form-dominant-withdrawn.md` (finding #5); `evidence_index.json` itself intentionally untouched (coordinator-owned; regeneration inputs documented in `dec-004`). `SENTINEL_FAIL` remains open by design — this task produces the honest regeneration inputs for the coordinator's `evidence_index.json`, not a self-asserted verdict change to that shared, coordinator-owned file. WSL up (`.venv-wsl`, main-checkout venv used by all worktrees via `bin\oc-py`/`bin\oc-pytest`).

**Unrelated prior handoff below (Gate 0/1/2 work, different task/session, inherited from base branch history — left intact for that context, not part of this task):**
