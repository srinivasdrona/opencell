...
The pre-registered primary outcome was `critique_score`: a 5-gate GPT-5.5 critique pass scored each diff CLEAN=1.0, WITH-CHANGES=0.5, DO-NOT-MERGE=0.0. The five gates (canonical in `docs/prompts/CRITIQUE_DIMER_PORT.md`) score the diff itself: **Gate 1 — Preservation** (no pre-existing assertion is weakened, deleted, or replaced); **Gate 2 — Rule 6 + Rule 7** (sibling chassis builders v3/v4/v5/v6 still construct, no schema dual-declaration of a port across `protein.counts` and `protein.enzyme_counts`); **Gate 3 — Schema split** (dimer-port `complex` channel is wired through the topology, not shoehorned into a substrate-side read); **Gate 4 — Tests** (per-process chassis-fixture, strict-zero unit test if present, and whole-chassis integration all pass under `py -3.12 -m pytest`); **Gate 5 — Fail-fast** (missing input raises at construction or first call, never silently returns `{}` or zeroes). A WITH-CHANGES on any gate halves the score; a DO-NOT-MERGE on any gate zeroes it.
...
A quick note on which rubric this references. The 5-gate `CRITIQUE_DIMER_PORT` rubric introduced above scores each individual diff (preservation / sibling-builder + schema / schema-split / tests / fail-fast). Sitting above it is a separate 4-gate *meta-rubric* (`docs/prompts/PREFIX_V2_VALIDATION_RUBRIC.md`) that classifies critique findings to decide which slot of the framework to evolve. Its gates: **Gate 1 — Inversion concreteness** (did Beat 4 declare ≥2 concrete, falsifiable failure modes rather than throwaway phrasing?); **Gate 2 — Verification evidence strength** (did the VERIFICATION block back each named mode with an executable probe — command, exit code, schema hash, pytest output — rather than narrative reassurance?); **Gate 3a — Verification falsified by critique** (a critique finding on a mode the codex *did* name, meaning Gate 2 evidence was weak or fabricated); **Gate 3b — Failure mode not named** (a critique finding on a mode the codex's Beat 4 never imagined, meaning the prefix's imagination ceiling is too low for this class of bug). The split between 3a and 3b is load-bearing: 3a means tighten the probe rubric for that mode (probe-rigor miss, fix at the template layer), 3b means add an explicit Rule the agent can now invert against (imagination miss, fix at the prefix or rule-list layer). Without the split, every critique finding would route to "write a new rule," which is how rule lists balloon into checklists that no one reads.
...
- **A rule against successful-looking deferrals.** Rule 8's decision tree includes "if the computation requires inputs the process doesn't yet receive, declare L2.1 N/A." This is correct but exploitable: a codex can claim N/A for any process whose fix is hard, and the framework as it stands has no probe-rigor rule to distinguish "honest topology gap" from "lazy skip." Critique can catch this case-by-case; a mechanical **Gate 6** on the validation rubric (alongside the existing 1/2/3a/3b) — say, *N/A claim must cite a specific missing observable by name plus a one-paragraph proposal for the harness extension that would un-block it* — cannot yet be written generically.
...
## Addendum: two things this post earlier got wrong or under-said

Added 2026-06-01 evening after re-reading the draft.

**1. Hardened prompts produced legible failure fingerprints, which is what made the scout-and-fanout pattern possible.** The post above describes the L2 reuse as "we ported the template family onto L2," and that is true at the artifact level — but it understates what changed at the *workflow* level. Once L2.1 replay tests were written under hardened prompts (Rules 1–4 of the L2 template specifically: observable coverage manifest, integer-exact compare, WID-length alignment guard, per-tick state isolation), every RED came back with a structured first-mismatch fingerprint of the form `tick=N, observable=substrates, index=K, oc_val=X, karr_val=Y, diff=±Z`, plus a `substrate_wids[K]=<WID>` line. Before hardening, REDs failed with mostly-narrative `AssertionError` messages and the human had to chase down which observable + index combination diverged. After hardening, the failures were *machine-readable*. That mechanical legibility is what unlocked `docs/prompts/SCOUT_TEMPLATE_L2_REPLAY.md`: a scout agent could read the L2 status surface, classify each RED by `fix_class` (`dimer-port` / `harness` / `biology` / `data-archaeology` / `unclear`), and emit a structured `SCOUT_REPORT.json` that the orchestrator turned into a parallel codex fanout — six to twelve children per wave. **The taxonomy was a consequence of the hardened prompts, not a prerequisite.** Today's three afternoon agents are evidence of the same property: rna_decay returned `Class A — init-state seeding, hidden RNA-pool + RNG-stream state`, metabolism v3-slot returned `Class C-irreducible — harness projection plus missing evolveState machinery`, and the divergence between them is exactly what the SCOUT-style `fix_class` routing exists to separate. If the failures had still been narrative, both would have read as "the test failed at tick 0, please fix it," and the only sensible orchestrator move would have been to hand both to the same agent. With legible fingerprints, the orchestrator can route one to a harness-extension follow-up and the other to a biology-port follow-up, and neither agent wastes tokens re-discovering which kind of bug it is.

**2. The honest sequencing correction.** In a prior session retro I described this campaign as a five-move framework with the dimer-port template at step 5 — i.e., as the culmination of a longer methodological arc that began with critique loops, then naked-numbers-lint, then deliberate-action-prefix, then fix-templates, then dimer-port. That framing reads cleanly but reverses cause and effect. The actual sequence is the opposite: **the dimer-port bug class was step 1**, the empirical anchor that motivated everything else. The first thing that happened was the TR-R2 critique loop visibly failing to converge on a real bug; the *response* to that failure (across the next 36 hours) produced the deliberate-action prefix, the fix template, the case-specific preservation directive, Rule 6, Rule 7, and finally — much later — the L2 reuse and the SCOUT taxonomy. Each of those was downstream of the dimer-port discovery. The L2.1 replay rules (1–8) were not borrowed from a prior framework; they were freshly minted from the L2 pilot critiques, with the dimer-port artifact family as the template *shape*. The five-move retro framing collapsed the timeline backward and made it look like dimer-port was the answer to a question that already existed. It was actually the question that produced the framework. Worth noting because the framing matters for anyone trying to adopt the same approach without an empirical anchor of their own — the lesson is "find the bug class your team keeps re-introducing, then write the framework downstream of it," not "adopt this framework and apply it to whatever you have."
